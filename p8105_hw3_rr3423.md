...
### Creating a single-panel plot showing the 24-hour activity time courses for each day

``` r
accel_df %>% 
  mutate(activity_count = )
```

    ## # A tibble: 50,400 × 6
    ##     week day_id day    weekday_vs_weekend activity_minute activity_count
